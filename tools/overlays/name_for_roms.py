#!/usr/bin/env python3
"""Give every cartridge a same-named .art copy so auto-load finds it.
...
Matching is by normalized title key (same normalization as
merge_sources.py): exact key match first, then unique containment either
way (so "pole" matches "Pole Position (1983)(GCE)"). Ambiguous
